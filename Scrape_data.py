...
def stream_data(welcome):
    for word in welcome.split(" "):
        yield word + "  "  # yeild keyword is used to create a generator function, A type of function that is memory efficient and can be used as iterator object
        time.sleep(0.1)
    ...
    if st.button('Search'):
        if Config_topic.topic:
            ...
            if news_data:
                ...
                st.download_button(label="Download CSV", data=df.to_csv(index=False), file_name='../news_data_{}.csv'.format(Config_topic.news_Number),
                                   mime='text/csv')
            else:
